-- TODO BEFORE RELEASE (THIS IS A BETA VERSION!)
-- PROBLEMA: CONTEGGIA IL GIRO INIZIALE A ZERO!!!
-- commentare: la gara si deve avviare fisicamente da dentro la soglia altrimenti si visualizza il mess "too far"
-- commentare: posizionare il quad sulla linea di start ed allontanarsi seguendo il senso di percorrenza della pista fino a che l'rssi non equipara la soglia rssi graficata
-- commentare: la gara inizia con il gas al messaggio waiting for gas
-- commentare: il timer è fatto per avviarsi se armato ed rssi > -98. Se non hai settato il fondo scala a - 100 dello stick del throttle, potresti avere problemi ad avviare il timer. Ti invito a trimmare la tua radio!
-- commentare: il timer parte quando armato e thr > 0. si ferma per i parziali (in caso di riarmo per crash) e se non riattivato segna il tempo di gara. per resettare e ricominciare la gara, seguire la guida.
-- Implementare salvataggio log contenente statistiche, data e modello su scheda SD
-- Spostare i metodi dei tasti da dentro i metodi race e setting e delegarli al metodo run
-- Se timer parte ed RSSI è giò triggato, viene conteggiato il primo giro a zero. EVITARE
...
-- Thanks: My MilanoFPV's m8s for testing and RCdiy.ca
-- Premise: the script, and the hardware itself, introduce  uncertainty.
--  -The system erros is more or less repeatable so a relative measure of time lap is good.
--  -To be considered as a nice tool to measure improvements on a circuit.
--  -Said that, this is provided as not suitable for official race.
...
-- Suitable for OpenTX Version: 2.1.8 to 2.2.3
...
-- Laps count is triggered by a coded logical switch activated by RSSI threshold.
...
-- Place the accompanying folder "LamTmr" within Radio' SD Card: /SCRIPTS/TELEMETRY/
-- Enter to the model menu and scroll to the "Display" page. Assign the LapTmr script to
-- a screen and select LapTmr. From the radio main menu, long press "page" to access the
-- screens and select LapTmr.
-- To play: Throlle down, arm your kwad, 3, 2, 1, Throttle up! ...and it will count.
-- Enjoy your race!

--------------------- Configuration (to be edit at will) ---------------------------------
-- Set the arm switch assignment as desired
-- You can be assigned to physical switches (sa to sh) or locigal ones (ls1 to ls32)
local ArmSwitch = "sa"
...
--------------------- Usage --------------------------------------------------------------
-- Place your kwad at a reasonable distance. Orientate the antenna as follow (or specular):
--
--            .##########################(##
--                                    .*(#%%#.
--                                    ##%%#%@#
...
--            .....#************************//*//////////////////((#(/*
--
-- Run the script and set the RSSI threshold. Don't move yourself from choosen position!
-- Arm your kwad and, as soon as it pass near to the start point, a lap will be counted.
-- Button usage:
